...
---𝗡𝗩𝗟 takes exactly two parameters the value to check for null and the replacement value
...
-- NVL is more specific to Oracle databases--- Oracle methodology
 ...
 --  𝗖𝗢𝗔𝗟𝗘𝗦𝗖𝗘 can take more than two parameters. It returns the first non-null value among its arguments.
SELECT studentid, studentname, COALESCE(age, 25) as new_age, grade FROM student;

--  𝗡𝗨𝗟𝗟𝗜𝗙 is used to compare two expressions , if they are equal, NULLIF returns NULL; otherwise, it returns the first expression.
